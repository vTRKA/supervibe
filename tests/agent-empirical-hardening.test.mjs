...
test("all agents have generated per-agent eval packs and score heatmap rows", () => {
  ...
  assert.equal(records.agents.length, 97);
...
test("empirical hardening validator covers evals, heatmap, stack fixtures, and Russian corpus", () => {
  ...
  assert.equal(report.checkedAgents, 97);
  assert.equal(report.evalPacks, 97);
  assert.ok(report.evalCases >= 276);
  assert.equal(report.heatmapRows, 97);
