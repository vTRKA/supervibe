...
import { detectNextPhase } from '../scripts/lib/evolve-state-detector.mjs';
...
async function makeProject(name, opts = {}) {
  ...
  if (opts.versionSeen) {
    await writeFile(join(root, '.claude', 'memory', '.evolve-version'), opts.versionSeen);
  }
  if (opts.invocations) {
    const lines = opts.invocations.map(e => JSON.stringify(e)).join('\n') + '\n';
    await writeFile(join(root, '.claude', 'memory', 'agent-invocations.jsonl'), lines);
  }
  if (opts.overrides) {
    const lines = opts.overrides.map(e => JSON.stringify(e)).join('\n') + '\n';
...
async function makePlugin(name, opts = {}) {
  ...
  await writeFile(join(root, '.claude-plugin', 'plugin.json'), JSON.stringify({ name: 'evolve', version: opts.version || '1.7.0' }));
  if (opts.upstreamCache) {
    await writeFile(join(root, '.claude-plugin', '.upgrade-check.json'), JSON.stringify(opts.upstreamCache));
  }
  if (opts.staleAgents) {
    await mkdir(join(root, 'agents'), { recursive: true });
    for (let i = 0; i < opts.staleAgents; i++) {
      await writeFile(join(root, 'agents', `stale-${i}.md`),
...
test('detector: project-not-scaffolded fires when .claude/agents/ + CLAUDE.md absent', async () => {
  ...
  assert.strictEqual(r.proposed.command, '/evolve-genesis');
...
test('detector: upstream-behind takes priority over project-not-scaffolded', async () => {
  ...
  const r = await detectNextPhase(project, plugin);
  assert.strictEqual(r.proposed.command, '/evolve-update');
...
test('detector: version-bump-unacked fires when project saw older plugin version', async () => {
  const project = await makeProject('p-old', { scaffolded: true, versionSeen: '1.6.0' });
  const plugin = await makePlugin('plugin-new', { version: '1.7.0' });
  const r = await detectNextPhase(project, plugin);
  assert.strictEqual(r.proposed.command, '/evolve-adapt');
  assert.match(r.proposed.reason, /1\.6\.0.*1\.7\.0|1\.7\.0/);
});

test('detector: stale-artifacts fires only when ≥3 stale files exist', async () => {
  const project = await makeProject('p-stale', { scaffolded: true, versionSeen: '1.7.0' });
  const plugin = await makePlugin('plugin-stale', { version: '1.7.0', staleAgents: 5 });
  const r = await detectNextPhase(project, plugin);
  assert.strictEqual(r.proposed.command, '/evolve-audit');
  assert.strictEqual(r.proposed.signal, 'stale-artifacts');
});

test('detector: stale under threshold (<3) does not trigger', async () => {
  const project = await makeProject('p-stale-2', { scaffolded: true, versionSeen: '1.7.0' });
  const plugin = await makePlugin('plugin-stale-2', { version: '1.7.0', staleAgents: 2 });
  const r = await detectNextPhase(project, plugin);
...
test('detector: override-rate fires when >5% over last ≥10 entries', async () => {
  const overrides = [];
  for (let i = 0; i < 12; i++) overrides.push({ override: i < 2, ts: new Date().toISOString() });
  const project = await makeProject('p-or', {
    scaffolded: true, versionSeen: '1.7.0', overrides,
  });
...
test('detector: pending-evaluation fires when latest invocation has no outcome', async () => {
  const project = await makeProject('p-pending', {
    scaffolded: true,
    versionSeen: '1.7.0',
    invocations: [{ agent_id: 'someone', task_summary: 't', confidence_score: 9, ts: new Date().toISOString() }],
  });
  const plugin = await makePlugin('plugin-pending', { version: '1.7.0' });
  const r = await detectNextPhase(project, plugin);
  assert.strictEqual(r.proposed.command, '/evolve-evaluate');
...
test('detector: pending-evaluation does NOT fire when latest has outcome', async () => {
  const project = await makeProject('p-acked', {
    scaffolded: true,
    versionSeen: '1.7.0',
    invocations: [{ agent_id: 'someone', task_summary: 't', confidence_score: 9, outcome: 'accept', ts: new Date().toISOString() }],
  });
...
test('detector: all-green when nothing fires', async () => {
  const project = await makeProject('p-clean', { scaffolded: true, versionSeen: '1.7.0' });
...
test('detector: report contains all 7 checks even on first-trigger short-circuit', async () => {
  ...
  for (const expected of [
    'upstream-behind', 'version-bump-unacked', 'project-not-scaffolded',
    'underperformers', 'stale-artifacts', 'override-rate-high', 'pending-evaluation',
  ]) {
