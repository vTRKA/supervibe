...
| MCP/browser automation | local admin screen only when needed for UI smoke | cookies, private payloads, private screenshots | user approval before private data capture |
...
- Use the T1 failing tests as red phase evidence, then implement only the listed files and keep unrelated billing behavior untouched.
...
- Commit suppressed until backend verification passes and review confirms only scoped files changed.
...
- [ ] **Step 1: Implement typed client and UI state changes after red phase**
- Use the T1 client failing test as red phase evidence, keep the dashboard layout stable, and avoid unrelated visual refactors.
...
- Commit suppressed until UI verification passes and review confirms only scoped files changed.
...
- Red phase evidence from T1-T3 failing tests has been captured and repaired before release begins.
...
**Real-agent receipt-backed handoff:** none in this example because backend and frontend depend on the same API contract. Add worker batches only after disjoint write sets and runtime-issued workflow receipts are available.

**Subagent-Driven batches:** prohibited for durable production work in this example.
