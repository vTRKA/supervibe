...
test('creates config.json for prototype dir without one', async () => {
  ...
  await mkdir(join(root, '.supervibe', 'artifacts', 'prototypes', 'foo'), { recursive: true });
  await writeFile(join(root, '.supervibe', 'artifacts', 'prototypes', 'foo', 'index.html'), '<html></html>');

  const result = await migratePrototypeConfigs({ projectRoot: root });
  assert.equal(result.created.length, 1);

  const cfg = JSON.parse(await readFile(join(root, '.supervibe', 'artifacts', 'prototypes', 'foo', 'config.json'), 'utf8'));
...
test('skips dirs that already have config.json', async () => {
  const root = await mkdtemp(join(tmpdir(), 'mig-'));
  await mkdir(join(root, '.supervibe', 'artifacts', 'prototypes', 'bar'), { recursive: true });
  await writeFile(join(root, '.supervibe', 'artifacts', 'prototypes', 'bar', 'config.json'), '{"target":"web","viewports":[]}');
...
test('skips reserved dirs (_design-system, _brandbook)', async () => {
  ...
  await mkdir(join(root, '.supervibe', 'artifacts', 'prototypes', '_brandbook'), { recursive: true });
...
test('idempotent — second run does nothing', async () => {
  const root = await mkdtemp(join(tmpdir(), 'mig-'));
  await mkdir(join(root, '.supervibe', 'artifacts', 'prototypes', 'baz'), { recursive: true });

  await migratePrototypeConfigs({ projectRoot: root });
  const stat1 = await stat(join(root, '.supervibe', 'artifacts', 'prototypes', 'baz', 'config.json'));

  await new Promise(r => setTimeout(r, 10));
  const result2 = await migratePrototypeConfigs({ projectRoot: root });
  assert.equal(result2.created.length, 0);

  const stat2 = await stat(join(root, '.supervibe', 'artifacts', 'prototypes', 'baz', 'config.json'));
