...
test("release-facing version surfaces are synchronized to 2.0.3", async () => {
  ...
  assert.match(opencode, /version:\s*"2\.0\.3"/);
  assert.match(readme, /\*\*v2\.0\*\*/);
  assert.match(readme, /plugin v2\.0\.3 initialized/);
  assert.match(changelog, /## \[2\.0\.3\] - 2026-04-30/);
});
...
test("getting-started local install examples use the current 2.0.3 cache path", async () => {
  const text = await readFile("docs/getting-started.md", "utf8");
  assert.match(text, /plugins\/cache\/local\/supervibe\/2\.0\.3/);
