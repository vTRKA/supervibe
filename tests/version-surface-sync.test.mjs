...
test("release-facing version surfaces are synchronized to 2.0.5", async () => {
  ...
  assert.match(opencode, /version:\s*"2\.0\.5"/);
  assert.match(readme, /\*\*v2\.0\*\*/);
  assert.match(readme, /plugin v2\.0\.5 initialized/);
  assert.match(changelog, /## \[2\.0\.5\] - 2026-04-30/);
});
...
test("getting-started local install examples use the current 2.0.5 cache path", async () => {
  const text = await readFile("docs/getting-started.md", "utf8");
  assert.match(text, /plugins\/cache\/local\/supervibe\/2\.0\.5/);
