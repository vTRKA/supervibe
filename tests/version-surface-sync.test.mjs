...
test("release-facing version surfaces are synchronized to 2.0.7", async () => {
  ...
  assert.match(opencode, /version:\s*"2\.0\.7"/);
  assert.match(readme, /\*\*v2\.0\*\*/);
  assert.match(readme, /plugin v2\.0\.7 initialized/);
  assert.match(changelog, /## \[2\.0\.7\] - 2026-05-01/);
});
...
test("getting-started local install examples use the current 2.0.7 cache path", async () => {
  const text = await readFile("docs/getting-started.md", "utf8");
  assert.match(text, /plugins\/cache\/local\/supervibe\/2\.0\.7/);
