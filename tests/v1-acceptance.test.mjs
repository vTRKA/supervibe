...
test('plugin manifest exists at canonical path with required fields', async () => {
  ...
  assert.ok(existsSync(manifestPath), 'manifest must exist at .claude-plugin/plugin.json');
  const data = JSON.parse(await readFile(manifestPath, 'utf8'));
  for (const field of ['name', 'description', 'version']) {
    assert.ok(field in data, `missing required field: ${field}`);
  }
  assert.strictEqual(data.name, 'evolve');
});
...
test('all 11 confidence rubrics present and validate', async () => {
  ...
  const expected = [
    ...
    'prototype', 'research-output', 'agent-quality', 'skill-quality', 'rule-quality', 'brandbook'
  ];
...
test('all 6 questionnaires present and parse', async () => {
  ...
  const files = (await readdir(dir)).filter(f => f.endsWith('.yaml'));
  assert.ok(files.length >= 6, `expected ≥6 questionnaires, found ${files.length}`);
  for (const file of files) {
    const content = await readFile(join(dir, file), 'utf8');
    const data = parseYaml(content);
    assert.ok(data.id, `${file}: missing id`);
    assert.ok(Array.isArray(data.questions), `${file}: missing questions array`);
  }
});
...
test('full reference stack-pack present with all required parts', async () => {
  ...
  assert.ok(existsSync(join(packDir, 'manifest.yaml')), 'manifest.yaml missing');
  assert.ok(existsSync(join(packDir, 'claude', 'settings.json')), 'claude/settings.json missing');
  assert.ok(existsSync(join(packDir, 'claude', 'CLAUDE.md.tpl')), 'claude/CLAUDE.md.tpl missing');
  assert.ok(existsSync(join(packDir, 'husky', 'pre-commit')), 'husky/pre-commit missing');
  assert.ok(existsSync(join(packDir, 'husky', 'pre-push')), 'husky/pre-push missing');
  assert.ok(existsSync(join(packDir, 'configs', 'lint-staged.config.js')), 'configs/lint-staged missing');
});
...
test('atomic packs present (≥5)', async () => {
  ...
  assert.ok(packs.length >= 5, `expected ≥5 atomic packs, found ${packs.length}`);
});

test('stack-pack settings.json has comprehensive deny-list (≥30 entries)', async () => {
  const settingsPath = join(ROOT, 'stack-packs', 'laravel-nextjs-postgres-redis', 'claude', 'settings.json');
  const data = JSON.parse(await readFile(settingsPath, 'utf8'));
  const deny = data.permissions.deny;
  assert.ok(deny.length >= 30, `expected ≥30 deny entries, found ${deny.length}`);

  const mustHave = [
    'Bash(git stash:*)', 'Bash(git push --force:*)', 'Bash(git reset --hard:*)',
    'Bash(rm -rf:*)', 'Bash(dropdb:*)', 'Bash(redis-cli FLUSHALL:*)',
    'Bash(php artisan migrate:fresh:*)'
  ];
  for (const entry of mustHave) {
    assert.ok(deny.includes(entry), `deny-list missing critical entry: ${entry}`);
  }
});

test('all 4 docs present', async () => {
  for (const doc of ['getting-started.md', 'skill-authoring.md', 'agent-authoring.md', 'rule-authoring.md']) {
...
test('all 33+ skills have valid trigger-clarity descriptions', async () => {
  const { checkTriggerClarity } = await import('../scripts/lib/trigger-clarity.mjs');
...
test('registry generates and includes all artifact types', async () => {
  ...
  const registry = parseYaml(await readFile(join(ROOT, 'registry.yaml'), 'utf8'));
  assert.ok(Object.keys(registry.agents).length >= 33, 'registry should have ≥33 agents');
  assert.ok(Object.keys(registry.skills).length >= 33, 'registry should have ≥33 skills');
  assert.ok(Object.keys(registry.rules).length >= 16, 'registry should have ≥16 rules');
  assert.ok(Object.keys(registry['stack-packs']).length >= 1, 'registry should have ≥1 stack-pack');
  assert.ok(Object.keys(registry['confidence-rubrics']).length >= 11, 'registry should have ≥11 rubrics');
});
