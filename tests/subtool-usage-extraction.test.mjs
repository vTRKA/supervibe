...
test('detects code-search references', () => {
  const out = extractSubtoolUsage('Run scripts/search-code.mjs --query "payment idempotency"');
  assert.equal(out['code-search'], 1);
});

test('detects code-graph callers', () => {
  const out = extractSubtoolUsage('Verified blast: scripts/search-code.mjs --callers "processPayment"');
...
test('detects memory file path references', () => {
  const out = extractSubtoolUsage('Read .claude/memory/decisions/2026-04-foo.md');
  assert.equal(out.memory, 1);
});

test('zero on agent that did not use any tool', () => {
  const out = extractSubtoolUsage('I implemented the feature without searching anything.');
