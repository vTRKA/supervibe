...
export const DEFAULT_WORKFLOW_TRIGGER_FIXTURES = Object.freeze([
  ...
  },
  {
    ...
    expected: { intent: "continue_plan", command: "/supervibe-plan --loop-ready --from-brainstorm", minConfidence: 0.9 },
  },
  {
    id: "plan-execution-goes-to-loop-atomization",
    phrase: "run it",
    context: { lastCompletedPhase: "plan", artifacts: { plan: true } },
    expected: { intent: "atomize_plan", command: "/supervibe-loop --atomize-plan <plan-path> --user-approved-plan", minConfidence: 0.9 },
  },
  {
    id: "user-approved-loop-ready-plan-atomize-ru",
    phrase: "разбей план на атомарные задачи",
    context: { artifacts: { plan: true, planReviewPassed: true } },
    expected: { intent: "atomize_plan", command: "/supervibe-loop --atomize-plan <plan-path> --user-approved-plan", minConfidence: 0.88 },
  },
  {
    id: "worktree-run-en",
  ...
  },
  {
    ...
    context: {
      recentAssistantOutput: [
        ...
        "Next phase: loop-atomization",
        "Next command: /supervibe-loop --atomize-plan <plan-path> --user-approved-plan",
        "Next skill: supervibe:writing-plans",
        "Stop condition: ask-before-graph-creation",
        "Question: Step 1/1: split the plan into atomic work items and an epic?",
        "END_NEXT_STEP_HANDOFF",
      ].join("\n"),
    },
    expected: { intent: "atomize_plan", command: "/supervibe-loop --atomize-plan <plan-path> --user-approved-plan", minConfidence: 0.95 },
  },
]);
