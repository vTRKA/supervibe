...
export function createIdeBridgeDescriptor({
  ...
  const entryUrl = `${baseUrl}/?token=${encodeURIComponent(token)}`;
  return {
    schemaVersion: 1,
    ...
    actions: {
      previewFirst: true,
      tokenHeader: "x-supervibe-ui-token",
    ...
    hostHints: [
      ...
      "Use POST /api/action only after showing preview and passing the token header.",
    ],
  };
}
