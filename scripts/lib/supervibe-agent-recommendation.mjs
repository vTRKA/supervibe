...
function renderManagedInstruction({ hostSelection, fingerprint, agentProfile, recommendedAgents, optionalAgents }) {
  ...
  return [
    ...
    "- Example: `запусти индексирование rag/codegraph` -> `node <resolved-supervibe-plugin-root>/scripts/build-code-index.mjs --root . --resume --no-embeddings --graph --max-files 200 --max-seconds 120 --health --json-progress`.",
    "- If the catalog returns `SUPERVIBE_COMMAND_MATCH`, run that exact command from the project root and do not search the whole project for command docs first.",
    "",
    "## Update Path",
