...
export async function performUpstreamCheck(pluginRoot) {
  ...
  const upstreamConfig = git(pluginRoot, ['rev-parse', '--abbrev-ref', '--symbolic-full-name', '@{u}']);
  let trackingRef = upstreamConfig.ok ? upstreamConfig.stdout : `origin/${result.currentBranch}`;

  // Refresh remote refs
  const fetch = git(pluginRoot, ['fetch', '--tags', '--prune', '--quiet'], FETCH_TIMEOUT_MS);
  if (!fetch.ok) {
  ...
  if (head.ok && upstream.ok && head.stdout !== upstream.stdout) {
    const behindCount = git(pluginRoot, ['rev-list', '--count', `HEAD..${trackingRef}`]);
    if (behindCount.ok) result.behind = parseInt(behindCount.stdout, 10) || 0;
  }

  // Latest tag on tracked branch (purely informational)
  const tag = git(pluginRoot, ['describe', '--tags', '--abbrev=0', trackingRef]);
...
/**
 * Spawn a detached child process that runs the upstream check in the
 * background. Returns immediately. The child writes the cache for the
 * next session to consume.
 */
export function spawnBackgroundCheck(pluginRoot) {
  try {
    const scriptPath = fileURLToPath(new URL('../evolve-upgrade-check.mjs', import.meta.url));
