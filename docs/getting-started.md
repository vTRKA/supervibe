...
cp -r ~/dev/supervibe ~/.claude/plugins/cache/local/supervibe/2.0.8

# Windows (PowerShell):
mkdir $HOME\.claude\plugins\cache\local\supervibe\2.0.8
xcopy /E /I "D:\ggsel projects\supervibe" "$HOME\.claude\plugins\cache\local\supervibe\2.0.8"

# Or symlink (avoids re-copy on updates):
# Linux/Mac:
ln -s ~/dev/supervibe ~/.claude/plugins/cache/local/supervibe/2.0.8
# Windows (admin shell):
mklink /D "$HOME\.claude\plugins\cache\local\supervibe\2.0.8" "D:\ggsel projects\supervibe"
...
- Check `~/.claude/plugins/cache/local/supervibe/2.0.8/.claude-plugin/plugin.json` exists
...
1. Confirm path: `ls ~/.claude/plugins/cache/local/supervibe/2.0.8/.claude-plugin/plugin.json`
...
- **Plugin manifest now requires `agents:[]` array** for nested agent dirs to work
  - Manifest auto-updated; ensure your install path has v2.0.8
...
- **Action**: re-symlink to v2.0.8 dir, restart Claude Code
