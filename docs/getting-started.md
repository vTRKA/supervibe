...
test suite; `npm run check` stays in developer hooks/CI.
...
cp -r ~/dev/supervibe ~/.claude/plugins/cache/local/supervibe/2.0.16

# Windows (PowerShell):
mkdir $HOME\.claude\plugins\cache\local\supervibe\2.0.16
xcopy /E /I "D:\ggsel projects\supervibe" "$HOME\.claude\plugins\cache\local\supervibe\2.0.16"

# Or symlink (avoids re-copy on updates):
# Linux/Mac:
ln -s ~/dev/supervibe ~/.claude/plugins/cache/local/supervibe/2.0.16
# Windows (admin shell):
mklink /D "$HOME\.claude\plugins\cache\local\supervibe\2.0.16" "D:\ggsel projects\supervibe"
...
- Check `~/.claude/plugins/cache/local/supervibe/2.0.16/.claude-plugin/plugin.json` exists
...
1. Confirm path: `ls ~/.claude/plugins/cache/local/supervibe/2.0.16/.claude-plugin/plugin.json`
...
- **Plugin manifest now requires `agents:[]` array** for nested agent dirs to work
  - Manifest auto-updated; ensure your install path has v2.0.16
...
- **Action**: re-symlink to v2.0.16 dir, restart Claude Code
