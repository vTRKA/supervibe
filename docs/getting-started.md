...
cp -r ~/dev/supervibe ~/.claude/plugins/cache/local/supervibe/2.0.68

# Windows (PowerShell):
mkdir $HOME\.claude\plugins\cache\local\supervibe\2.0.68
xcopy /E /I "C:\path\to\supervibe" "$HOME\.claude\plugins\cache\local\supervibe\2.0.68"

# Or symlink (avoids re-copy on updates):
# Linux/Mac:
ln -s ~/dev/supervibe ~/.claude/plugins/cache/local/supervibe/2.0.68
# Windows (admin shell):
mklink /D "$HOME\.claude\plugins\cache\local\supervibe\2.0.68" "C:\path\to\supervibe"
...
- Check `~/.claude/plugins/cache/local/supervibe/2.0.68/.claude-plugin/plugin.json` exists
...
1. Confirm path: `ls ~/.claude/plugins/cache/local/supervibe/2.0.68/.claude-plugin/plugin.json`
...
- **Plugin manifest now requires `agents:[]` array** for nested agent dirs to work
  - Manifest auto-updated; ensure your install path has v2.0.68
...
- **Action**: re-symlink to v2.0.68 dir, restart Claude Code
