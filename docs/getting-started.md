...
cp -r ~/dev/supervibe ~/.claude/plugins/cache/local/supervibe/2.1.7

# Windows (PowerShell):
mkdir $HOME\.claude\plugins\cache\local\supervibe\2.1.7
xcopy /E /I "C:\path\to\supervibe" "$HOME\.claude\plugins\cache\local\supervibe\2.1.7"

# Or symlink (avoids re-copy on updates):
# Linux/Mac:
ln -s ~/dev/supervibe ~/.claude/plugins/cache/local/supervibe/2.1.7
# Windows (admin shell):
mklink /D "$HOME\.claude\plugins\cache\local\supervibe\2.1.7" "C:\path\to\supervibe"
...
- Check `~/.claude/plugins/cache/local/supervibe/2.1.7/.claude-plugin/plugin.json` exists
...
1. Confirm path: `ls ~/.claude/plugins/cache/local/supervibe/2.1.7/.claude-plugin/plugin.json`
...
- Manifest auto-updated; ensure your install path has v2.1.7
...
- New: `scripts/search-memory.mjs` CLI
  - **Action**: re-symlink to v2.1.7 dir, restart Claude Code
