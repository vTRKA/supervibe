...
| Visual chat explanation of system or task logic | `/supervibe-preview --visual-explanation` | `supervibe:writing-plans`, `supervibe:design-intelligence`, `supervibe:preview-server` | `supervibe-orchestrator`, `ux-ui-designer`, `quality-gate-reviewer` | text-first summary, compact table or ASCII scheme, optional browser preview for UI/prototype evidence, Mermaid fallback, no hidden implementation claims | `visual-chat-explanation-required` |
...
When the user asks to understand logic, task flow, architecture, or audit process, the answer should include a compact visual artifact when it improves comprehension. Text-first summaries are preferred for ordinary workflow schemes: use a stage map, compact table, or improvised ASCII diagram. Browser previews are optional only for real UI/prototype/browser evidence; Mermaid remains fallback/export for terminal-only contexts. Every visual explanation needs a short text fallback so it remains useful in clients that do not render previews.

Text-first visual summary: compact stage map or table, no color-only status, and beginner-readable text fallback. Browser preview packets, when used for actual UI/prototype evidence, must include a local preview URL/path. Mermaid fallback must still include `accTitle:` and `accDescr:`.
