...
When the user asks to understand logic, task flow, architecture, or audit process, the answer should include a compact visual artifact when it improves comprehension. Browser-first preview is preferred for non-expert comprehension; Markdown tables are acceptable when comparison is clearer; Mermaid or ASCII is fallback/export for terminal-only contexts. Every visual explanation needs a short text fallback so it remains useful in clients that do not render previews.

Browser-first visual packet: local preview URL/path, decision cards or flow lanes, no color-only status, and beginner-readable text fallback. Mermaid fallback must still include `accTitle:` and `accDescr:`.
