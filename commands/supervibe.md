...
Trigger router for the full workflow chain: brainstorm -> loop-ready plan -> user-approved graph -> safe run -> optional verify/review. It also handles work-item questions, status, dashboard output, integration readiness, saved views, structured queries, local reports, autonomous replay evals, deferred work, guided forms, interactive command palette, sync bundle export/import, notifications, stop/resume, and trigger diagnostics (`--diagnose-trigger`, `--why-trigger`) before any mutating action.
...
- Plan ready: `Step N/M: create graph from this plan, revise, run deeper review, or stop?`
- Deeper review passed: `Step N/M: split the reviewed plan into atomic work items and an epic?`
...
Every producer result must also expose `NEXT_USER_ACTIONS[]` and a visible Next User Decision before progressing: create graph or continue, revise, exclude/defer, run or rerun specialist review, inspect readiness, or stop and keep the artifact. Natural-language plan review and review-loop requests route to `/supervibe-plan --review`, not audit or execution; normal graph creation uses `/supervibe-loop --atomize-plan <plan-path> --user-approved-plan`.
...
Plan/pre-plan continuation is fail-closed. A current explicit user answer is required after each visible handoff question; older broad consent does not answer a newly emitted `NEXT_STEP_HANDOFF`. Normal plan execution routes require the user-approved graph choice before atomization, epic creation, autonomous loop execution, version bump, commit, push, or cleanup. Optional deeper review remains explicit and must pass before using review-specific evidence claims.
...
Any claim that this command actually invoked another Supervibe command, skill, agent, reviewer, worker, validator, or external tool must be backed by a runtime-issued workflow receipt created with `node <resolved-supervibe-plugin-root>/scripts/workflow-receipt.mjs issue ...`. Showing a route, a next-choice card, or the normal `/supervibe-plan --loop-ready` to `/supervibe-loop --atomize-plan <plan-path> --user-approved-plan` handoff does not need receipts. Hand-written receipts are untrusted. Agent, worker, and reviewer receipts must include `hostInvocation.source` and `hostInvocation.invocationId` from a real host dispatch; command or skill receipts must not substitute for specialist output. Scoped current-run receipts are required for active delegated-command/handoff completion claims; old global receipts are diagnostic only and cannot authorize a new agent-owned output. Durable artifacts produced by this command must stay linked through `.supervibe/memory/workflow-invocation-ledger.jsonl` and `artifact-links.json`; run `npm run validate:workflow-receipts` and `npm run validate:agent-producer-receipts` only at explicit verification, strict review, or release gates before claiming delegated work or produced artifacts are complete.
