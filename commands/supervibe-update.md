...
This command must load its executable profile from `scripts/lib/command-agent-orchestration-contract.mjs` and follow `rules/command-agent-orchestration.md`. The profile is the source of truth for `ownerAgentId`, `agentPlan`, `requiredAgentIds`, default `real-agents` mode, and `agent-required-blocked` behavior. The normal update path is a command-owned utility exception: `/supervibe-update` runs in `utility-no-agent` mode and may mutate the managed plugin checkout through the updater, install dependencies, rebuild registry/state, and run install doctor without specialist agent fanout or runtime agent receipts.

Before durable work or completion claims, run `node <resolved-supervibe-plugin-root>/scripts/command-agent-plan.mjs --command /supervibe-update` and follow the printed `SUPERVIBE_COMMAND_AGENT_PLAN`. The expected normal plan is `EXECUTION_MODE: utility-no-agent`, `REQUIRED_AGENTS: none`, `PARALLEL_AGENT_DISPATCH_REQUIRED: false`, `RECEIPT_GATE: not-required-for-utility-command`, `DURABLE_WRITES_ALLOWED: true`, `CALLABLE_AGENT_SOURCES: none`, `CALLABLE_AGENTS_READY: true`, `SCOPED_RECEIPT_GATE: false`, and `MISSING_CALLABLE_AGENTS: none`. Use the updater and install-doctor output as the command evidence.

Only invoke real host agents when the user explicitly requests `--verify-agents`, review, recovery, or delegated specialist output. In those explicit agent modes, run the same command-agent plan and then use `spawn_agent` through `node <resolved-supervibe-plugin-root>/scripts/agent-invocation.mjs log ...` with runtime receipts. In Codex, follow printed `CODEX_SPAWN_PAYLOAD_RULES` and `CODEX_SPAWN_PAYLOADS`: forked payloads must set `fork_context=true`, must omit `agent_type`, `model`, and `reasoning_effort`, and must encode the Supervibe logical role in `message`. `inline` is diagnostic/dry-run only. Do not emulate specialist agents, and do not let command or skill receipts substitute for agent, worker, or reviewer output.
