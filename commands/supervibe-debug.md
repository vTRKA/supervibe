...
This is the missing entry-point in the audit→strengthen→evaluate loop: `/supervibe-audit` says "agent X is weak", `/supervibe-strengthen` proposes edits — but neither tells you WHY a specific invocation failed. `/supervibe-debug` answers that.

## Invocation forms

### `/supervibe-debug <invocation-id>` — by ID from telemetry

If user has the invocation ID (e.g., from `/supervibe-audit` output or `agent-invocations.jsonl` directly).

### `/supervibe-debug <agent-id>` — debug the most recent failure of an agent

Auto-resolves the most-recent invocation where confidence < 8 OR override = true OR outcome = failed.

Examples:
- `/supervibe-debug laravel-developer`
- `/supervibe-debug creative-director`

### `/supervibe-debug` — debug the latest failure across all agents

No args: scan `agent-invocations.jsonl` for the most recent failure (any agent), debug it.

### `/supervibe-debug --replay <invocation-id>` — re-dispatch on same task
...
### `/supervibe-debug --trace <invocation-id>` — full trace (no replay)
...
### `/supervibe-debug --all-failed` — list all recent failures
...
|---|---|---|
| `stale-context` | Agent referenced paths/symbols that don't exist | Project context drifted; run `/supervibe-adapt` |
...
| `ambiguous-task` | Override rationale mentions "unclear request" | User needs better /supervibe-brainstorm pre-step |

4. **Search for prior similar failures:**
   ```bash
   supervibe:project-memory --query "<agent> <task-keywords> failure"
   ```
   If matched → quote the prior fix; offer to reapply it.

5. **Dispatch root-cause-debugger** for deep analysis when classification is ambiguous (≥2 patterns possible). The debugger uses `supervibe:systematic-debugging` skill: symptom → 3 hypotheses → evidence per hypothesis → narrowed root cause.

6. **Propose fix** based on classification:

   - `stale-context` → suggest `/supervibe-adapt` to refresh project context
   - `missing-skill` → list skill candidates; offer to add to agent's Skills list
   - `wrong-approach` → suggest `/supervibe-strengthen <agent>` with focus on Decision tree
   ...
   - `ambiguous-task` → suggest re-running through `/supervibe-brainstorm` first

7. **Optional replay** (if `--replay`):
   - Re-dispatch same agent on same task
   - Compare new output to old output side-by-side
   - Score each via `supervibe:confidence-scoring`
...
| Replay returns same low score | Suggest deeper investigation: read agent file + skill files; consider `/supervibe-strengthen` |
...
=== Proposed fix ===
Run: /supervibe-strengthen laravel-developer
...
- When telemetry shows agent succeeded — there's nothing to debug; check whether you actually want strengthening (use `/supervibe-score agent-quality`).
