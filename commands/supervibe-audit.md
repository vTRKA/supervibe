...
Default audit mode is read-only/no-write diagnostic. It may inspect existing receipts and telemetry, but it must not issue new receipts, repair ledgers, rebuild memory, or mutate generated state. A read-only audit may report "diagnostic only"; it must not claim that real host agents were completed unless trusted receipt-bound host-agent telemetry already exists.

If the user explicitly asks for a trusted 10/10 agent-system audit, use receipt-writing mode. In that mode the orchestrator must state the mutation boundary before writing and include `MUTATED: .supervibe/memory/agent-invocations.jsonl` plus every workflow receipt path that was issued. The required maturity gate is:

```bash
node <resolved-supervibe-plugin-root>/scripts/supervibe-agent-maturity.mjs
```

The gate must verify strict host-agent telemetry, Code RAG/CodeGraph readiness, dynamic specialist questions, continuation contracts, and receipt trust before any 10/10 maturity claim. Receipt-writing mode is the only mode that may create new host-agent telemetry during `/supervibe-audit`.
