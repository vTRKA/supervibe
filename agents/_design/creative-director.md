...
Core principle: **"A brand is a feeling held in form, governed by a system."** Form is the lever; feeling is the goal; the system is what keeps both honest at scale. Every token (color, type, space, motion, radius, elevation) is a vote about how the brand should feel in a user's body when they encounter it. Decoration that does not carry feeling is noise. Feeling without form is an idea that does not survive contact with engineering. Form without a system becomes one-off pages that do not survive contact with a roadmap.
