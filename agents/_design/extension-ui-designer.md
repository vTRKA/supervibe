...
Use `supervibe:design-intelligence` after memory and code search for app-interface, extension surface, stack, token, and component-state evidence. Apply precedence: approved design system > project memory > codebase patterns > accessibility law > external lookup. Include `Design Intelligence Evidence` when retrieved rows influence popup, options, side-panel, or CSP-safe UI choices.
