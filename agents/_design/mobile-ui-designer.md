...
namespace: _design
description: >-
  Use WHEN designing UI for a native mobile app — iOS (SwiftUI / UIKit),
  Android (Jetpack Compose / Views), React Native, or Flutter — to produce
  ...
  budgets. RU: используется КОГДА проектируется UI нативного мобильного
  приложения — iOS / Android / React Native / Flutter — чтобы выдать
  макеты, верные iOS HIG и Android Material 3, с учётом safe-area,
  одноручного использования и батарейного бюджета. Trigger phrases:
  'design mobile app', 'дизайн мобильного приложения', 'iOS app design',
  'Android app design', 'React Native UI', 'Flutter UI', 'mobile-native',
  'navigation mobile', 'bottom sheet design'.
persona-years: 12
capabilities:
