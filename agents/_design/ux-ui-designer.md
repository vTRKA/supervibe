...
2. **Artifact mode gate**: if existing `.supervibe/artifacts/prototypes/`, `.supervibe/artifacts/mockups/`, or prior specs match the brief and the user did not explicitly say continue existing vs new from scratch, ask one question before treating old files as source. Prior files are evidence, not permission to reuse.
