...
BEFORE starting any non-trivial task — feature, bug fix, refactor, design decision. Especially BEFORE invoking `evolve:brainstorming` or `evolve:writing-plans`.
...
1. Verify `.claude/memory/` exists; if not, propose `evolve:add-memory` is needed first
...
- `evolve:add-memory` — writes new memory entries (this skill READS only)
- `agents/_meta/memory-curator` — maintains memory hygiene
- `evolve:brainstorming` — primary consumer (reads memory before exploring)
- `evolve:writing-plans` — secondary consumer (reads patterns before planning)
